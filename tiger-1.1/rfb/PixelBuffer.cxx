...
#include "tiger-1.1/rfb/PixelBuffer.h"
...
void FullFramePixelBuffer::copyRect(const Rect &rect, const Point &move_by_delta) {
  ...
  if (!drect.enclosed_by(getRect())) {
    fprintf(stderr, "Destination rect %dx%d at %d,%d exceeds framebuffer %dx%d",
  ...
  if (!srect.enclosed_by(getRect())) {
    fprintf(stderr, "Source rect %dx%d at %d,%d exceeds framebuffer %dx%d",
...
ManagedPixelBuffer::checkDataSize() {
  ...
  if (datasize < new_datasize) {
    fprintf(stderr, "reallocating managed buffer (%dx%d)", width_, height_);
    if (data) {
      delete [] data;
      datasize = 0; data = 0;
    }
