...
/*
 *  Copyright (C) 2010-2012, 2014-2015 D. R. Commander.  All Rights Reserved.
    ...
    int quality;
{
    ...
    int pitch;
    unsigned char *tmpbuf = NULL, *yuvImage = NULL;
    ...
    x264_param_t param;
    x264_t *encoder = NULL;
    int pw, ph, stride[3], pixelFormat = TJPF_RGB;
    ...
    if (ps == 2) {
        ...
        if (ps == 4)
            pixelFormat += 2;
        if (rfbServerFormat.bigEndian && ps == 4) {
    ...
    yuvImage = (unsigned char *)malloc(tjBufSizeYUV(w, h, TJSAMP_420));
    if (!yuvImage) {
        rfbLog("Memory allocation error\n");
        goto bailout;
    }

    pw = PAD(w, 2);  ph = PAD(h, 2);
    ...
    if (tjEncodeYUVPlanes(t->j, srcbuf, w, pitch, h, pixelFormat, plane,
                          stride, TJSAMP_420, 0) < 0) {
